...
def load_module():
    spec = importlib.util.spec_from_file_location("backfill_single_gpu", SCRIPT_PATH)
...
def test_passes_when_requested_plugin_matches_snapshot_canonical(
    ...
    mod = load_module()
    hust = "a46abb7ae68acc13a4fc5870db98619b3f97c6e0"  # pragma: allowlist secret
    plugin = "f430530ada2c0c2ec2f925606494bc95a474d9c8"  # pragma: allowlist secret
...
def test_raises_when_requested_plugin_disagrees_with_snapshot_canonical(
    ...
    mod = load_module()
    hust = "a46abb7ae68acc13a4fc5870db98619b3f97c6e0"  # pragma: allowlist secret
    canonical = "f430530ada2c0c2ec2f925606494bc95a474d9c8"  # pragma: allowlist secret
    divergent = "03a12f9bddd944952bd029c6b62e23d68fa3a28e"  # pragma: allowlist secret
    with patch.object(
...
def test_passes_on_snapshot_miss_when_no_canonical_known() -> None:
    ...
    mod = load_module()
    hust = "0000000000000000000000000000000000000000"  # not in any snapshot  # pragma: allowlist secret
    plugin = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef"  # pragma: allowlist secret
...
def test_allow_override_permits_mismatch_and_records_audit_entry(
    ...
    with (
        ...
        # Pass via override.
        mod.assert_plugin_commit_consistent(hust, override_value, allow_override=True)
        # Record the audit entry as cmd_run would.
        mod.record_plugin_override(state, hust, canonical, override_value)
