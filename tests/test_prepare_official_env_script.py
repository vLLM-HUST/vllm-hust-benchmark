...
def test_strict_host_orchestration_owns_explicit_multicard_idle_proof() -> None:
    result = _run_bash(
        _source_run_official_functions(
            """
            ...
            VLLM_HUST_STRICT_HOST_PEAK_HBM_FILE=/workspace/repeat/strict-host-peak-hbm.json
            resolve_npu_smi_bin() { return 1; }
            configure_single_card_ascend_device
            """
        ),
    ...
    assert result.returncode == 0
    assert "delegated to strict host orchestration" in result.stdout
