...
def test_workload_contract_defaults_match_active_target_registry() -> None:
    """Admission must not reject the effective values generated by the registry."""
    registry = build_registry(REPO_ROOT)
    for target in registry["targets"]:
        if target["status"] != "active":
            continue
        scenario = target["workload"]["name"]
        defaults = _official_defaults_for_scenario(scenario)["server"]
        for key, expected in defaults.items():
            assert target["server_parameters"][key] == expected, (
                target["target_id"],
                key,
            )
