...
> **已被 V4.6 正式交付测试方案取代。** 本文仅保留 legacy leaderboard/nightly 工程健康和 历史操作说明，不得用于声明 V4.6 A1-A4 验收通过。

## 1. 概述

本方案定义国产芯片（以华为昇腾 Ascend 910B2 为首批验证对象）上大模型推理引擎的**关键性能指标体系**、**标准化评测场景**与**完整可复现的基线测试流程**。

目标：

1. 建立可量化、可复现、可对比的推理性能评测标准；
1. 覆盖在线服务（Online Serving）、离线吞吐（Offline Throughput）、延迟（Latency）三大类典型业务场景；
1. 形成基线数据集，作为后续优化工作的参考锚点；
1. 提供端到端操作手册，确保任意团队成员可独立复现全部基线数据。
...
| 峰值显存      | Peak Memory      | MB       | 单次运行期间观测到的峰值显存占用        |
...
| `long_context_ttft_p95_stable`           | 长上下文 TTFT P95 是否稳定 |
| `long_context_ttft_p99_stable`           | 长上下文 TTFT P99 是否稳定 |
| `long_context_tpot_p95_stable`           | 长上下文 TPOT P95 是否稳定 |
| `long_context_tpot_p99_stable`           | 长上下文 TPOT P99 是否稳定 |
...
当前文档暂仅体现**单卡官方基线场景**。当前代码中单卡基线场景共 **9 个**，覆盖三大类业务；多卡 `*-2chip`、`*-4chip`、`*-8chip` 变体已在 registry 和
spec 中注册，但不在本方案正文展开。
...
| 3   | `prefix-repetition-online` | Serve      | 长上下文      | 固定 4096     | 固定 256   | 200     |
| 4   | `instructcoder-online`     | Serve      | 代码助手      | 数据集分布    | 数据集分布 | 2048    |
| 5   | `visionarena-online`       | Serve      | 多模态        | 数据集分布    | 数据集分布 | 1000    |
| 6   | `agent-research-online`    | Serve      | AI 研究 Agent | Trace 分布    | Trace 分布 | 32      |
| 7   | `sharegpt-throughput`      | Throughput | 离线批处理    | 数据集分布    | 数据集分布 | 200     |
| 8   | `sonnet-throughput`        | Throughput | 离线批处理    | Sonnet 数据集 | 数据集分布 | 200     |
| 9   | `random-latency`           | Latency    | 延迟 SLO      | 固定 1024     | 128        | batch=8 |
...
- 当前 registry 默认参数：dataset_name=prefix_repetition, input_len=4096, output_len=256, num_prompts=200
...
#### 3.2.6 `agent-research-online`（AI 研究 Agent 场景）

- 使用 EvoScientist 多 Agent 研究系统 trace 模拟真实研究助手请求
- 数据集：`scripts/traces/evoscientist-workload-custom.jsonl`
- 核心考察指标：复杂多轮 Agent 工作负载下的 TTFT、吞吐和稳定性

#### 3.2.7 `sharegpt-throughput`（离线吞吐场景）

- 离线批处理模式，不限 QPS，引擎自行调度以最大化吞吐
- 核心考察指标：Throughput TPS

#### 3.2.8 `sonnet-throughput`（轻量离线吞吐场景）

- 较短文本的离线吞吐测试
- 输入约 550 tokens，输出约 150 tokens
- 核心考察指标：Throughput TPS

#### 3.2.9 `random-latency`（固定 batch 延迟场景）
...
| 系统内存 | ≥ 128GB                                                                        |
| 磁盘空间 | ≥ 100GB（模型权重 + 数据集）                                                   |
| 卡数配置 | 本方案正文暂仅体现 1 卡（单卡）官方基线；2/4/8 卡变体由 registry/spec 另行维护 |
...
| ----------- | --------------------------------------------------------------------------------------- |
| 推理引擎    | vLLM v0.18.0                                                                            |
| Ascend 插件 | vllm-ascend v0.18.0                                                                     |
| Python      | 3.11.x                                                                                  |
| PyTorch     | 2.9.x                                                                                   |
| CANN        | 8.0+                                                                                    |
| 评测模型    | Qwen/Qwen2.5-14B-Instruct、Qwen/Qwen2.5-Coder-14B-Instruct、Qwen/Qwen2.5-VL-7B-Instruct |
| 量化方式    | 无（FP16 基线）                                                                         |
...
1. **模型加载**：下载或使用本地缓存的模型权重
1. **服务启动**：启动 vLLM server，等待健康检查通过
1. **负载生成**：按场景配置发送请求（Poisson 到达或全量并发）
1. **指标采集**：逐请求记录 TTFT、ITL、output tokens 等
1. **结果聚合**：计算均值、中位数、P95、P99 等统计量
1. **数据导出**：生成标准化 JSON 结果文件
...
```json
{
  ...
  "hardware": {
    ...
    "memory_per_chip_gb": 64.0,
    "total_memory_gb": 64.0
  },
  "model": {
    "canonical_id": "qwen2.5-14b-instruct",
  ...
  "constraints": {
    "accountable_scope": { "declared_baseline_engine": "vllm" },
    "metrics": { "single_chip_effective_utilization_pct": null }
  },
  "versions": { "protocol": "N/A", "backend": "N/A", "core": "N/A", "benchmark": "<version>" },
  "metadata": { "submitter": "official-ascend-baseline", "data_source": "reference-vllm-ascend-benchmark" },
  "same_spec": {
...
| official baseline spec           | `docs/official-baselines/*.json`                                      | `vllm-hust-benchmark`          | official runner、same-spec、baseline coverage | 当前官方目标固定为 Ascend + `v0.18.0`                          |
...
当前代码里“场景支持面”的单一事实来源是 `src/vllm_hust_benchmark/data/official_scenarios.json`，并由 `registry.py` 加载为
`ScenarioDefinition`。

| benchmark type | 场景数 | 当前支持场景                                                                                                                          |
| -------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| `serve`        | 6      | `sharegpt-online`, `random-online`, `prefix-repetition-online`, `instructcoder-online`, `visionarena-online`, `agent-research-online` |
...
| Wrapper                              | 覆盖范围                                                                                            | 备注                                                 |
| ------------------------------------ | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `run` / `build-command` / `run-both` | 面向 registry，原则上覆盖本文列出的 9 个单卡场景；registry 中多卡变体也可通过对应 scenario 名称访问 | 新增场景时，通常先验证这层是否已足够                 |
...
1. `leaderboard_compare.json` 是聚合派生产品，不是第二种 submission schema。
...
1. **L2（按需门禁）**：路径自动触发（变更命中 benchmark/推理热点路径）+ 手工触发（workflow_dispatch 输入 `scenario_list`），抽样来源优先复用
   `official_scenarios.json`。
1. **L3（基线与发布）**：继续使用现有 `run-official-ascend-baselines.yml`，保持 `submissions/** -> push-to-hf.yml`
   自动同步，GitHub snapshots 为 website 第一优先级。

#### 10.4.4 权限与资源治理

1. **权限控制**：高成本 benchmark 触发（L2/L3）应限制在维护者可控入口（workflow_dispatch + 受控变量）上。
1. **运行环境标准化**：继续沿用 pinned runtime 和 prepare 脚本提供的环境修复能力，L1/L2 也应复用统一环境准备逻辑。
1. **稳定性与容错**：保留 repeat + canonical selector 机制用于 L3，L1/L2 可引入单次自动重试但不得掩盖持续性退化。

#### 10.4.5 验收标准（CI/CD 维度）

1. PR 默认至少包含 1 个性能 smoke 验证，且结果可追溯。
1. 高风险改动可自动或手动触发 L2，且结果参与门禁。
1. L3 定时运行持续产出可用基线，并能被 L1/L2 消费。
1. GitHub snapshots 与 HF sync 的发布链路在失败时有明确告警和回滚路径。
1. 触发权限、runner 资源与 secrets 管理有文档化约束并可审计。
...
1. 创建独立 conda 环境 `vllm-ascend-official-v0180`
1. 安装 vLLM v0.18.0 及其依赖
1. 安装 vllm-ascend v0.18.0
1. 锁定 torch / torch-npu / torchvision / torchaudio 版本
...
pip install torch-npu==2.9.0  # 匹配 CANN 版本
...
bash scripts/run-official-ascend-goal-baseline.sh \
  docs/official-baselines/official-ascend-jan-2026-v0180-random-latency-qwen25-14b-910b2.json
...
以下为在 Huawei 910B2 单卡上，使用 vLLM v0.18.0 + vllm-ascend v0.18.0 运行官方单卡场景获得的官方基线数据。

### 14.1 测试环境

| 项目 | 值 |
|------|-----|
| 硬件 | Huawei Atlas 800I A2, 910B2 × 1 |
...
| 文件路径 | 说明 |
|---------|------|
| `vllm-hust-benchmark/src/vllm_hust_benchmark/data/official_scenarios.json` | 官方场景注册表；本文暂仅展开单卡场景 |
...
将以下内容保存为 `run_baseline_core_text.sh`，可在 Ascend 机器上一键执行核心文本单卡基线测试。完整单卡官方基线以 `docs/official-baselines/*.json` 和 matrix runner 为准。
...
#==============================================================================
# 国产芯片（昇腾 910B2）大模型推理核心文本基线测试一键脚本
...
本方案中所有基线数据来自 `vllm-hust-benchmark` 仓库 `submissions/` 目录下的 `official-ascend-jan-2026-v0.18.0-*` 系列提交，由自动化 CI 在 Atlas 800I A2 服务器上实测产出。
