...
当前文档暂仅体现**单卡官方基线场景**。当前代码中单卡基线场景共 **9 个**，覆盖三大类业务；多卡 `*-2chip`、`*-4chip`、`*-8chip` 变体已在 registry 和 spec 中注册，但不在本方案正文展开。
...
2. 安装 vLLM v0.18.0 及其依赖
3. 安装 vllm-ascend v0.18.0
4. 锁定 torch / torch-npu / torchvision / torchaudio 版本
