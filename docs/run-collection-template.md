...
1. [设计原则](#1-设计原则)
2. [Artifact 目录约定](#2-artifact-目录约定)
3. [采集文件清单](#3-采集文件清单)
4. [运行模板](#4-运行模板)
5. [单次运行脚本：run-single-repetition.sh](#5-单次运行脚本-run-single-repetitionsh)
6. [批次运行脚本：run-campaign-repetitions.sh](#6-批次运行脚本-run-campaign-repetitionsh)
7. [环境采集脚本：collect-run-artifact.sh](#7-环境采集脚本-collect-run-artifactsh)
8. [校验脚本：validate-run-artifact.sh](#8-校验脚本-validate-run-artifactsh)
9. [完整使用示例](#9-完整使用示例)
10. [验收清单](#10-验收清单)
...
T18 定义了三个 campaign 的版本锁定和 spec 文件。T19 提供运行这些 spec 的**采集模板**——确保每次运行产出的 artifact 满足 producer 消费要求，并且重跑不会丢失旧数据。
...
所有最终 artifact 写入 `submissions/` 目录（与现有历史数据一致）。
运行时状态写入 `.benchmarks/<campaign-prefix>/<artifact-dir-name>/`，
完成后仅将最终产物复制到 `submissions/`。
...
| `env_vars` | 关键环境变量 | PATH, LD_LIBRARY_PATH, PYTHONPATH, HF_HOME, VLLM_CACHE_ROOT, ASCEND_* |
