...
The local `vLLM-HUST标准交付测试方案_V4.6.pdf` is the authority for project delivery acceptance.
The machine-readable declaration is
`src/vllm_hust_benchmark/data/acceptance_v4_6.json`.
The PDF identity is pinned by SHA-256 in that declaration so a replacement document cannot be
silently treated as the same acceptance version.
...
- Performance labels and release/formal requests enter the independent evaluator after authentication,
  idempotency, contract, resource, and evidence preflight.
...
metrics.
Until those contracts are reviewed and admitted, no existing leaderboard or Perfgate result may be
promoted to V4.6 acceptance.

Repository administrators must remove the deleted Smoke, Regression, and Merge Gate check names
from Rulesets or branch protection before merging the workflow changes.
