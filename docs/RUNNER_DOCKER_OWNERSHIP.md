...
`job.container` 目前无法通过该脚本完成创建前校验。在 GitHub Actions 能可靠写入上述 label 和设备映射以前，不得把 `job.container` 作业调度到
`linux-aarch64-a2b3-pool`。

### 验收

合并前仍需在真实 runner 上分别验证成功、失败和取消路径，并确认两轮 watchdog 扫描后正确标记的容器仍存活；错误 label 或错误设备映射的告警记录在 #125。
