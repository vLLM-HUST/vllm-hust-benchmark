...
DEFAULT_VLLM_API_KEY = "dummy"  # pragma: allowlist secret
...
def run_trace(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            f"[WARN] Full agent failed ({e}), falling back to direct LLM multi-turn trace"
        )
        handler.close()
...
def _run_direct_llm(args: argparse.Namespace, handler) -> int:
    ...
    print(
        "[INFO] Running realistic multi-agent trace (simulating iterative research workflow)..."
    )
...
def main() -> None:
    ...
    if not check_service(args.base_url):
        ...
        print(
            "The trace infrastructure is ready - rerun this script when the service is up."
        )
        sys.exit(1)
