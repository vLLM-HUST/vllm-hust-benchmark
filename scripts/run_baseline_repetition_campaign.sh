...
for REP in 1 2 3; do
  NPU=$((REP - 1))
  PORT=$((BASE_PORT + NPU))
  PID=$(run_one "core-text-14b" "$REP" "$NPU" "$PORT" "$CORE_TEXT_SPEC" "$CORE_TEXT_MODEL")
  CORE_PIDS+=("$PID")
done
...
for REP in 1 2 3; do
  ...
  PID=$(run_one "coder-14b" "$REP" "$NPU" "$PORT" "$CODER_SPEC" "$CODER_MODEL")
  CODER_PIDS+=("$PID")
done
...
run_vision_sequential() {
  for REP in 1 2 3; do
    ...
    mkdir -p "$result_dir"
    echo "[baseline-campaign] launching vision-7b rep${REP} on NPU7 port${VISION_PORT} -> $result_dir"
    ...
    echo "[baseline-campaign] vision-7b rep${REP} exit=$?"
  done
}
