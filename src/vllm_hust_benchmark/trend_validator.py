...
def check_invalid_metrics(
    ...
    if throughput is None:
        reasons.append(
            "throughput_tps is null; latency workloads may omit it, other workloads require it"
        )
        critical = "latency" not in workload_name
    elif not isinstance(throughput, (int, float)) or throughput <= 0:
        metrics["throughput_tps"] = None
        reasons.append(
            "throughput_tps must be > 0; set it to null and retain the raw artifact"
        )
        critical = "latency" not in workload_name
    ...
    if error_rate is not None and (
        not isinstance(error_rate, (int, float)) or not 0 <= error_rate <= 1
    ):
        ...
        reasons.append(
            "error_rate is above 0.5; downgrade to experimental until investigated"
        )
...
def _schema_issues(
    entry: Mapping[str, Any], validator: Draft7Validator
) -> list[ValidationIssue]:
    return [
        ValidationIssue("SCHEMA_INVALID", error.message, _entry_id(entry))
        for error in sorted(
            validator.iter_errors(entry), key=lambda item: list(item.path)
        )
    ]


def _local_decision(
    entry: dict[str, Any], validator: Draft7Validator
) -> AdmissionDecision:
    ...
    if schema_issues:
        return AdmissionDecision(
            entry_id,
            "invalid",
            "Schema validation failed; fix the listed fields",
            tuple(schema_issues),
        )
    ...
    if not entry.get("trend_schema_version") or not entry.get("coverage_class"):
        issue = ValidationIssue(
            "LEGACY_NOT_ADMITTED",
            "Legacy entry has no trend coverage fields; migrate it before formal admission",
            entry_id,
            "warning",
        )
        return AdmissionDecision(
            entry_id, "excluded", "Legacy entry retained for provenance only", (issue,)
        )

    contract_errors: list[str] = []
    metadata = _nested(entry, "metadata")
    if requires_workload_config_contract(entry) or metadata.get(
        "workload_config_contract"
    ):
        contract_errors = validate_explicit_workload_config(entry)
    if contract_errors:
        issues = tuple(
            ValidationIssue("EFFECTIVE_CONFIG_INVALID", message, entry_id, "warning")
    ...
    if critical_invalid:
        issues = tuple(
            ValidationIssue("INVALID_METRIC", reason, entry_id)
    ...
    if (
        metric_reasons
        and any("throughput_tps" in reason for reason in metric_reasons)
        and "latency" in str(_nested(entry, "workload").get("name") or "").lower()
    ):
        issue = ValidationIssue(
            "LATENCY_THROUGHPUT_NOT_APPLICABLE",
            "random-latency may omit throughput_tps; keep the sanitized metric null and admit only as blocked",
            entry_id,
            "warning",
        )
    ...
    if coverage == "experimental":
        reason = (
            "W8A8/INT8 is outside the formal support matrix"
            if _is_w8a8(entry)
            else (entry.get("trend_reason") or "Experimental entry")
        )
        return AdmissionDecision(entry_id, "experimental", reason)
    if metric_reasons:
        return AdmissionDecision(entry_id, "experimental", "; ".join(metric_reasons))
    return AdmissionDecision(
        entry_id, "pending", "Awaiting cross-entry coverage checks"
    )
...
def _repeat_completeness(
    ...
    series = [
        candidate for candidate in entries if candidate.get("repeat_group") == group
    ]
    ...
    if len(indices) != len(set(indices)) or sorted(indices) != list(
        range(len(indices))
    ):
...
def _cross_entry_decision(
    entry: dict[str, Any],
    decision: AdmissionDecision,
    entries: list[dict[str, Any]],
    local: dict[str, AdmissionDecision],
) -> AdmissionDecision:
    ...
    if coverage == "full-matrix":
        if entry.get("point_role") != "checkpoint" or not entry.get("campaign_id"):
            issue = ValidationIssue(
                "MATRIX_METADATA_MISSING",
                "full-matrix requires campaign_id and point_role=checkpoint",
                entry_id,
                "warning",
            )
            return AdmissionDecision(entry_id, "blocked", issue.message, (issue,))
        ...
        if not group:
            return AdmissionDecision(
                entry_id, "experimental", "Single full-matrix run has no repeat_group"
            )
        complete, reason = _repeat_completeness(entry, entries)
        if not complete:
            issue = ValidationIssue(
                "MATRIX_REPEAT_INCOMPLETE", reason, entry_id, "warning"
            )
            return AdmissionDecision(entry_id, "blocked", issue.message, (issue,))
        ...
        if aggregate.get("count", 0) < 3:
            return AdmissionDecision(
                entry_id,
                "experimental",
                f"Insufficient repetitions: got {aggregate.get('count')}, need at least 3",
            )
        return AdmissionDecision(
            entry_id, "default", "Formal full-matrix series passed admission"
        )

    if coverage == "targeted-pair":
        if not entry.get("repeat_group") or not entry.get("canonical_aggregate"):
            issue = ValidationIssue(
                "PAIR_AGGREGATE_MISSING",
                "targeted-pair requires repeat_group and canonical_aggregate before formal admission",
                entry_id,
                "warning",
            )
            return AdmissionDecision(entry_id, "blocked", issue.message, (issue,))
        ...
        if not complete:
            issue = ValidationIssue(
                "PAIR_REPEAT_INCOMPLETE", reason, entry_id, "warning"
            )
            return AdmissionDecision(entry_id, "blocked", issue.message, (issue,))
        if (entry.get("canonical_aggregate") or {}).get("count", 0) < 3:
            return AdmissionDecision(
                entry_id,
                "experimental",
                "Insufficient repetitions: targeted-pair needs at least 3 samples per side",
            )
        ...
        counterpart = next(
            (
                ...
                if candidate.get("comparison_id") == comparison_id
                and candidate.get("point_role") != role
            ),
            None,
        )
        if counterpart is None:
            issue = ValidationIssue(
                "PAIR_HALF_MISSING",
                f"No comparable {('head' if role == 'baseline' else 'baseline')} entry for comparison_id={comparison_id!r}",
                entry_id,
                "warning",
            )
            return AdmissionDecision(entry_id, "blocked", issue.message, (issue,))
        if _pair_key(entry) != _pair_key(counterpart):
            issue = ValidationIssue(
                "PAIR_NOT_COMPARABLE",
                "Pair dimensions differ; align model, hardware, precision, workload, and topology",
                entry_id,
                "warning",
            )
            return AdmissionDecision(entry_id, "blocked", issue.message, (issue,))
        counterpart_complete, counterpart_reason = _repeat_completeness(
            counterpart, entries
        )
        ...
        if counterpart_decision is None or counterpart_decision.status != "pending":
            ...
            issue = ValidationIssue(
                "PAIR_COUNTERPART_NOT_ADMITTED",
                f"Counterpart status is {status}; resolve it before publishing the pair",
                entry_id,
                "warning",
            )
            return AdmissionDecision(entry_id, "blocked", issue.message, (issue,))
        return AdmissionDecision(
            entry_id, "default", f"Formal targeted-pair {role} passed admission"
        )
    return decision


def validate_entries(
    entries: Iterable[Mapping[str, Any]], *, schema_path: Path | None = None
) -> ValidationReport:
    ...
    report.decisions = [
        _cross_entry_decision(entry, local[_entry_id(entry)], copied, local)
        for entry in copied
    ]
    report.issues = [
        issue for decision in report.decisions for issue in decision.issues
    ]
    return report
